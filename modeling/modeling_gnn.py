...
class GATConvE(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_type, node_type, node_feature_extra,ep_1,ep_2, return_attention_weights=False):
        """
        x: [N, emb_dim]
        edge_index: [2, E]
        ...
        edge_embeddings=F.normalize(edge_embeddings, p=2, dim=-1)
        # num_nodes=x.shape[0]
        # L_node = torch.max(torch.norm(x, dim=1)).item()
        # L_edge = torch.max(torch.norm(edge_embeddings, dim=1)).item()
        sensitivity =1
        # D = torch.max(torch.bincount(edge_index[0])).item()
        edge_embeddings=edge_embeddings.to(x.device)
        edge_index=edge_index.to(x.device)
        # print(x.shape)
        # print(edge_embeddings.shape)
        # aggr_out = scatter(edge_embeddings, edge_index[1], dim=0, reduce="sum", dim_size=num_nodes)
        aggr_out = self.propagate(edge_index, x=x, edge_attr=edge_embeddings)
        ep=float(ep_1/4)
        aggr_out=agg_dp(aggr_out,ep_1,sensitivity)
...
class Decoder(nn.Module):
    def __init__(self, args, num_rels, h_dim):
        ...
        self.adversarial_temperature =1
        self.reg_param = 0.01
    ...
    def loss(self, scores):
        ...
        del positive_score,negative_score
