...
from transformers import (OpenAIGPTTokenizer, BertTokenizer, XLNetTokenizer, RobertaTokenizer,T5Tokenizer)
try:
...
class MultiGPUSparseAdjDataBatchGenerator(object):
    ...
    def __iter__(self):
        ...
        for a in range(0, n, bs):
            ...
            node_type_ids = batch_tensors1[1]
            edge_index, edge_type, pos_triples, neg_nodes = self.process_graph_data(edge_index, edge_type, node_type_ids)
            yield tuple([batch_qids, batch_labels, *batch_tensors0, *batch_lists0, *batch_tensors1, *batch_lists1, edge_index, edge_type,pos_triples, neg_nodes])
    ...
    def process_graph_data(self, edge_index, edge_type, node_type_ids):
        ...
        del _edge_index, _edge_type, _pos_triples, _neg_nodes
    ...
    def _process_one_graph(self, _edge_index, _edge_type, _node_type_ids):
        ...
        del pos_edge_type,pos_edge_index,num_corruption,num_edges,real_mask,real_drop_positions,real_drop_count,mask,drop_positions,drop_idxs,positions,is_special,_is_special,_edge_index_node_type
        return input_edge_index, input_edge_type, pos_triples, neg_nodes
...
class SDPGNP_DataLoader(object):
    ...
    def __init__(self, train_statement_path, train_adj_path,
        ...
        self.device0, self.device1,self.device2,self.device3 = device
    ...
    def load_resources(self, kg):
        # Load the tokenizer
        # try:
        #     tokenizer_class = {'bert': BertTokenizer, 'xlnet': XLNetTokenizer, 'roberta': RobertaTokenizer, 'albert': AlbertTokenizer,'T5': T5Tokenizer}.get(self.model_type)
        # except:
    ...
    def load_input_tensors(self, input_jsonl_path, max_seq_length):
        ...
        if use_cache:
            ...
            if self.model_type in ('lstm',):
                ...
                input_tensors = load_T5_xlnet_roberta_input_tensors(input_jsonl_path, max_seq_length, self.debug, self.tokenizer, self.debug_sample_size)
...
def load_T5_xlnet_roberta_input_tensors(statement_jsonl_path, max_seq_length, debug, tokenizer, debug_sample_size):
    class InputExample(object):
    ...
    examples = read_examples(statement_jsonl_path)
    features, concepts_by_sents_list = simple_convert_examples_to_features(examples, list(range(len(examples[0].endings))), max_seq_length, tokenizer)
    print('to大小：',tokenizer.vocab_size)
    example_ids = [f.example_id for f in features]
    *data_tensors, all_label = convert_features_to_tensors(features)
    return example_ids, all_label, data_tensors, concepts_by_sents_list