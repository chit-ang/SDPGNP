...
from modeling import modeling_mycode
...
def load_data(args, devices, kg):
    ...
    #########################################################
    dataset = data_utils.SDPGNP_DataLoader(args.train_statements, args.train_adj,
...
def construct_model(args, kg):
    ...
    if args.cxt_node_connects_all:
        n_etype += 2
    model = modeling_mycode.SDPGNP(args, args.encoder, k=args.k, n_ntype=n_ntype, n_etype=n_etype, n_concept=concept_num,
        concept_dim=args.gnn_dim,
        concept_in_dim=concept_in_dim,
        n_attention_head=args.att_head_num, fc_dim=args.fc_dim, n_fc_layer=args.fc_layer_num,
        p_emb=args.dropouti, p_gnn=args.dropoutg, p_fc=args.dropoutf,
        pretrained_concept_emb=cp_emb, freeze_ent_emb=freeze_ent_emb,
        init_range=args.init_range, ie_dim=args.ie_dim, info_exchange=args.info_exchange, ie_layer_num=args.ie_layer_num, sep_ie_layers=args.sep_ie_layers, layer_id=args.encoder_layer,ep_1=args.ep_1,ep_2=args.ep_2)
    return model
...
def calc_eval_accuracy(eval_set, model, loss_type, loss_func, debug, save_test_preds, preds_path,epoch_id):
    ...
    with torch.no_grad():
        for qids, labels, *input_data in tqdm(eval_set, desc="Dev/Test batch"):
            bs = labels.size(0)
            logits, link = model(*input_data,epoch=epoch_id)
    ...
    if save_test_preds:
        f_preds.close()
    return  n_corrects_acm / n_samples_acm
...
def train(args, resume, has_test_split, devices, kg):
    ...
    model.to(devices[1])
    model.lmgnn.mp.encoder_gnn.to(devices[2])
    model.lmgnn.mp.encoder_gnn.edge_encoder.to(devices[2])
    model.lmgnn.mp.encoder_gnn.gnn_layers.to(devices[2])
    model.lmgnn.concept_emb.to(devices[1])
    model.lmgnn.LLM.to(devices[3])
    ...
    total_time = 0
    model.train()
    tex='prompt/prompt_obqa_2_'+str(args.ep_1)+'.pt'
    print(tex)
    ...
    for epoch_id in trange(last_epoch + 1, args.n_epochs, desc="Epoch"):
        ...
        for qids, labels, *input_data in tqdm(train_dataloader, desc="Batch"):
            ...
            for a in range(0, bs, args.mini_batch_size):
                ...
                logits, link= model(*[x[a:b] for x in input_data],epoch=epoch_id)
                link_loss, pos_link_loss, neg_link_loss = link
                del pos_link_loss,neg_link_loss
                loss, n_corrects = calc_loss_and_acc(logits, labels[a:b], args.loss, loss_func)
                total_loss = (1.0 * loss + 0.3 * link_loss)
                # logits: [bs, nc]

                total_loss_acm += total_loss.item()
            ...
            noise_scale = args.max_grad_norm * np.sqrt(2 * np.log(1.25 / delta)) / 64
            if args.max_grad_norm > 0:
                nn.utils.clip_grad_norm_(model.parameters(), args.max_grad_norm)
                for name, p in model.named_parameters():
                    if 'classifier'  in name:  # 如果参数名称中包含'mlp'，则使用原始梯度更新
                        pass
                    else:
                        if p.grad is not None:
                            noise = torch.normal(0, noise_scale, size=p.grad.size(), device=p.grad.device)
        ...
        if epoch_id==20:
            torch.save(model.lmgnn.graph, tex)
            # torch.save(model.lmgnn.mp.encoder_gnn.graphs, 'graph_new_0.pt')

        torch.cuda.empty_cache()
        model.eval()
        preds_path = os.path.join(args.save_dir, 'dev_e{}_preds.csv'.format(epoch_id))
        dev_acc = calc_eval_accuracy(dev_dataloader, model, args.loss, loss_func, args.debug, not args.debug, preds_path,epoch_id)
        if has_test_split:
            preds_path = os.path.join(args.save_dir, 'test_e{}_preds.csv'.format(epoch_id))
            test_acc = calc_eval_accuracy(test_dataloader, model, args.loss, loss_func, args.debug, not args.debug, preds_path,epoch_id)
        else:
            test_acc = 0
...
def evaluate(args, has_test_split, devices, kg):
    ...
    preds_path = os.path.join(args.save_dir, 'dev_e{}_preds.csv'.format(epoch_id))
    dev_acc = calc_eval_accuracy(dev_dataloader, model, args.loss, loss_func, debug, not debug, preds_path,epoch_id)
    if has_test_split:
        # Evaluation on the test set
        preds_path = os.path.join(args.save_dir, 'test_e{}_preds.csv'.format(epoch_id))
        test_total_loss, test_acc = calc_eval_accuracy(test_dataloader, model, args.loss, loss_func, debug, not debug, preds_path,epoch_id)
    else:
        test_acc = 0
...
def get_devices(use_cuda):
    ...
    return device0, device1,device2,device3
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_dir', default=f'./saved_models/sdpgnp/', help='model output directory')
    ...
    parser.add_argument("--ep_1", default=0.1, type=int, help="Progation-DP.")
    parser.add_argument("--ep_2", default=0.1, type=int, help="CrossModel-DP.")
    # Data
    ...
    # Model architecture
    parser.add_argument('-k', '--k', default=5, type=int, help='The number of SDPGNP layers')
    ...
    parser.add_argument('--info_exchange', default=True, choices=[True, False, "every-other-layer"], type=utils.bool_str_flag, help="Whether we have the MInt operator in every SDPGNP layer or every other GreaseLM layer or not at all.")
    parser.add_argument('--ie_layer_num', default=1, type=int, help='number of hidden layers in the MInt operator')
    parser.add_argument("--sep_ie_layers", default=False, type=utils.bool_flag, help="Whether to share parameters across the MInt ops across differernt SDPGNP layers or not. Setting it to `False` means sharing.")
